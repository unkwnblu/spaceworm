...
export default function ProductsClient({ products }: { products: DBProduct[] }) {
      ...
      >
        {filtered.map((p) => (
          <tr key={p.id} className="hover:bg-zinc-50">
            ...
            <td className="px-4 py-3 text-xs text-zinc-700">{fmt(p.price)}</td>
            <td className="px-4 py-3">
              <div className="flex flex-wrap gap-1">
                {p.sizes.slice(0, 3).map((s) => (
                  <span key={s} className="border border-zinc-200 px-1.5 py-0.5 text-[9px] font-semibold uppercase text-zinc-500">
                    {s}
                  </span>
                ))}
                {p.sizes.length > 3 && (
                  <span className="text-[9px] text-zinc-400">+{p.sizes.length - 3}</span>
                )}
              </div>
            </td>
            <td className="px-4 py-3">
